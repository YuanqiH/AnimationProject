...
public:
	...
	Cylinder(const float height, const float radiusTop, const float radiusBottom, const vec4& colorTop, const vec4& colorBottom, const int segments = 16) 
	{
	...
	void generateVertices(const float height, const float radiusTop, const float radiusBottom, const vec4& colorTop, const vec4& colorBottom, const int segments = 16)
	{
	...
	void generateObjectBuffer() {
		...
		//Calc Array Sizes
		vertexArraySize = vertex_positions.size()*sizeof(vec4);
		colorArraySize = vertex_colors.size()*sizeof(vec4);
...
private:
	int nbSegments;
	std::vector<vec4> vertex_positions;
